...
# ## 이미지를 저장할 폴더 경로
# folder_path = 'C:/Users/User/Desktop/project4/my_image/images'
...
# def capture_owner_images(num_images=1) : ## num_images에 숫자를 입력한만큼 이미지 저장
#     ## 0은 기본 웹캠
...
#             ## 파일 이름 설정 및 저장
#             face_file = os.path.join(folder_path, f"my_face_{count+8735}.jpg")
...
## 이미지를 저장할 폴더 경로
folder_path = 'C:/Users/User/Desktop/project4/seo/images'
## 이미지 저장 폴더가 없다면 폴더 생성
if not os.path.exists(folder_path) :
    os.makedirs(folder_path)
    print('seo 폴더를 생성합니다.')
...
## 웹캠으로 얼굴 사진을 찍어 저장하는 함수
def capture_owner_images(num_images=2000) : ## num_images에 숫자를 입력한만큼 이미지 저장
    ## 0은 기본 웹캠
    cap = cv2.VideoCapture(0)
    ## haarcascade 알고리즘으로 얼굴 탐지
    face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
    ...
    while count < num_images :
        ...
        for (x, y, w, h) in faces :
            ...
            resized_face = cv2.resize(face, (512 , 512) )
            ## 파일 이름 설정 및 저장
            face_file = os.path.join(folder_path, f"my_face_{count}.jpg")
